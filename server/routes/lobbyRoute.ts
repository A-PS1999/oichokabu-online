...
import { Lobby as LobbyDB } from '../db/api';
import { Lobby as LobbySockets } from '../sockets';

const router = Router();

router.post('/api/lobby/create-game', checkLoggedIn, (request, response) =>
    LobbyDB.addGame(response.locals.user.id, request.body.roomName, request.body.playerCap, request.body.turnMax, request.body.betMax)
        .then(result => {
...
router.get('/api/lobby/lobbies', (_request, response) => {
...
router.get('/api/lobby/user-chips', checkLoggedIn, (_request, response) => {
    const userId = response.locals.user.id;
