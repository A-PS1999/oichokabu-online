...
export default function GameBoard() {
	...
	const dealerCards = (currentDealer?.cardBet ?? []) as ReadonlyArray<CardType>;

	return (
		<>
			{currentPhase === 'bettingPhase' ? <MakeBetForm /> : null}
			{(currentPhase === 'decideThirdCardPhase' || currentPhase === 'dealerCardsPhase') ? (
				<ThirdCardModal />
			) : null}
			{currentPhase === 'roundResults' ? <RoundResults /> : null}
			<div className="maingame">
				<div className="maingame__turninfo">
					<h2 className="maingame__turninfo__text">
						Turn: {currentTurn}/{turnMax}
					</h2>
				...
				{currentDealer ? (
					<>
						<div className="maingame__dealerinfo">
							<p>
								Dealer: <b>{currentDealer.username}</b>
							</p>
						</div>
						<div className="maingame__dealercards-container">
							...
							<CardsValueCounter cards={dealerCards} parentColumn={'D'} />
						</div>
					</>
				) : null}
				<div className="maingame__cardcolumn-container">
					{cardsOnBoard.length > 0 ? (
						cardsOnBoard.map((column, index) => {
							return <CardColumn key={index} column={column} columnIndex={index} />;
						})
					) : (
						<h2>Loading...</h2>
					)}
							...
							<Fragment key={player.id}>
								<div className="maingame__player">
									...
									<div key={player.id} className="maingame__player__playerinfo">
										<div className="maingame__player__playerinfo__username">
											{player.username}
										</div>
										<div className="maingame__player__playerinfo__chips">
											Chips: {player.chips}
										</div>
									</div>
								</div>
